...
# pylint: disable  MC80OmFIVnBZMlhsaUpqbWxvYzZjWGN6WXc9PTplODEyMjQ3Mg==
...
# noqa  MS80OmFIVnBZMlhsaUpqbWxvYzZjWGN6WXc9PTplODEyMjQ3Mg==
    ...
    # RAG-Anything multimodal processing configuration
    parser.add_argument(
        "--enable-multimodal-processing",
        action="store_true",
        default=get_env_value("ENABLE_MULTIMODAL_PROCESSING", True, bool),
        help="Enable multimodal document processing with RAG-Anything (default: from env or True)",
    )
    parser.add_argument(
        "--multimodal-parser",
        type=str,
        default=get_env_value("MULTIMODAL_PARSER", "mineru"),
        choices=["mineru", "docling"],
        help="Parser for multimodal document processing (default: from env or mineru)",
    )
    parser.add_argument(
        ...
        help="Parse method for multimodal documents (default: from env or auto)",
    )
    parser.add_argument(
        "--multimodal-parser-output-dir",
        type=str,
        default=get_env_value("MULTIMODAL_PARSER_OUTPUT_DIR", "./parser_output"),
        help="Output directory for multimodal parser results (default: from env or ./parser_output)",
    )
    ...
    parser.add_argument(
        ...
        help="Vision model binding type (default: from env or fallback to LLM_BINDING)",
    )
    parser.add_argument(
        "--vl-model",
        type=str,
        default=get_env_value("VL_MODEL", None),
        help="Vision model name (default: from env or fallback to LLM_MODEL)",
    )
    parser.add_argument(
        "--vl-binding-host",
        type=str,
        default=get_env_value("VL_BINDING_HOST", None),
        help="Vision model API endpoint (default: from env or fallback to LLM_BINDING_HOST)",
    )
    parser.add_argument(
        "--vl-binding-api-key",
        type=str,
        default=get_env_value("VL_BINDING_API_KEY", None),
        help="Vision model API key (default: from env or fallback to LLM_BINDING_API_KEY)",
    )
...
# noqa  Mi80OmFIVnBZMlhsaUpqbWxvYzZjWGN6WXc9PTplODEyMjQ3Mg==
    ...
    args.enable_multimodal_processing = get_env_value(
        "ENABLE_MULTIMODAL_PROCESSING", True, bool
    )
    ...
    # Vision Model configuration with fallback to LLM configuration
    args.vl_binding = get_env_value("VL_BINDING", None) or args.llm_binding
    args.vl_model = get_env_value("VL_MODEL", None) or args.llm_model
    args.vl_binding_host = get_env_value("VL_BINDING_HOST", None) or args.llm_binding_host
    args.vl_binding_api_key = get_env_value("VL_BINDING_API_KEY", None) or args.llm_binding_api_key
...
# fmt: off  My80OmFIVnBZMlhsaUpqbWxvYzZjWGN6WXc9PTplODEyMjQ3Mg==
