...
# pragma: no cover  MC80OmFIVnBZMlhsaUpqbWxvYzZkV3cyY2c9PTplY2ZjYzZlZQ==
...
# pylint: disable  MS80OmFIVnBZMlhsaUpqbWxvYzZkV3cyY2c9PTplY2ZjYzZlZQ==
...
# pragma: no cover  Mi80OmFIVnBZMlhsaUpqbWxvYzZkV3cyY2c9PTplY2ZjYzZlZQ==
...
# noqa  My80OmFIVnBZMlhsaUpqbWxvYzZkV3cyY2c9PTplY2ZjYzZlZQ==
    ...
    # Create vision_model_func for multimodal processing
    vision_model_func = None
    if args.enable_multimodal_processing:
        def create_vision_model_func(vl_binding: str):
            """
            Create vision model function based on binding type.
            Supports OpenAI, Azure OpenAI, Gemini, and Ollama vision models.
            """
            try:
                if vl_binding == "openai":
                ...
                elif vl_binding == "azure_openai":
                    from lightrag.llm.azure_openai import azure_openai_complete_if_cache

                    async def azure_vision_model_complete(
                        prompt,
                        system_prompt=None,
                        history_messages=None,
                        image_data=None,
                        ...
                        **kwargs,
                        ...
                        """Azure OpenAI vision model function"""
                        if history_messages is None:
                            history_messages = []

                        # If messages format is provided
                        if messages:
                            return await azure_openai_complete_if_cache(
                                args.vl_model,
                                "",
                    ...
                    return azure_vision_model_complete

                elif vl_binding == "gemini":
                    from lightrag.llm.gemini import gemini_complete_if_cache

                    async def gemini_vision_model_complete(
                        prompt,
                        system_prompt=None,
                        history_messages=None,
                        image_data=None,
                        ...
                        **kwargs,
                        ...
                        """Gemini vision model function"""
                        if history_messages is None:
                            history_messages = []

                        # Gemini handles multimodal content natively
                        # For now, use similar structure to OpenAI
                        if messages:
                    ...
                    return gemini_vision_model_complete

                elif vl_binding == "ollama":
                    from lightrag.llm.ollama import ollama_model_complete

                    async def ollama_vision_model_complete(
                        prompt,
                        system_prompt=None,
                        history_messages=None,
                        image_data=None,
                        ...
                        **kwargs,
                        ...
                        """Ollama vision model function (e.g., llava)"""
                        if history_messages is None:
                            history_messages = []

                        # Ollama supports vision through specific models like llava
                    ...
                    return ollama_vision_model_complete

                else:
                    logger.warning(
                        f"Unsupported vision model binding: {vl_binding}. "
                        "Vision model will not be available."
                    )
                    return None

            except ImportError as e:
                logger.error(f"Failed to import {vl_binding} vision model binding: {e}")
                return None

        vision_model_func = create_vision_model_func(args.vl_binding)
        if vision_model_func:
            logger.info(
                f"Vision model initialized: {args.vl_model} using {args.vl_binding} provider"
            )
        else:
            logger.warning(
